...
-- 3. Total Cases vs Population per country (note a person can get infected more than once)
...
-- 6. Top 10 Higuest death COUNT per continent
...
-- AROUND THE WORLD
-- 8. PER DATE: Cases, new cases, deaths per day (keep in mind a person can get infected again so 'total cases' are counting people who got infected more than once)
SELECT date, SUM(new_cases) AS Total_new_cases, 
SUM(total_cases), SUM(new_deaths) AS total_Deaths, 
...
-- 10. Adding a Rolling Total using 'New vaccines' to identify total of vaccines (this assume the 'total_vaccinations' column doesn't exist)
...
/* 11. I want to use the result of the rol_total_vaccin to calculate relation(percentage) with population.
